class User < ActiveRecord::Base
  ...
  # before_save :password_encrypted, :if => self.pass.present?
  before_save :password_encrypted
  ...
  def self.authenticate(email, password)
    ...
    if usr != nil &&
      usr.pass == Digest::SHA1.hexdigest(MY_APPLICATIONS['salt'] + password) then
      usr
    else
      return
    end
  ...
  def password_encrypted
    if self.pass.present?
      self.pass = Digest::SHA1.hexdigest(MY_APPLICATIONS['salt'] + self.pass)
    else
      return
    end
  end
